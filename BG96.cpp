...
int BG96::getCIMI(char* szCIMI, int nBufferSize)
{
	...
	for (int i=0; i<nNbLine; i++)
	{
		...
		char phoneNumber[16];
		memset(phoneNumber, 0x0, sizeof(phoneNumber));

		strcpy(phoneNumber, sLine);
		strcpy(szCIMI+1, phoneNumber+5);	/*skip 45006*/
		szCIMI[0] = '0';
		free(aLine[i]);
	}
...
#if 0
int BG96::getRejectCause(char* rejectCause)
{
	char    szATcmd[16];
	char    resBuffer[16];
	...
	if (0 == sendATcmd(szATcmd, resBuffer, sizeof(resBuffer), "+CEREG"))
	{
		...
		// CEREG=3ÀÇ °æ¿ì, return Çü½ÄÀº +CEREG:3,<stat>,<tac>,<ci>,<act>,<cause_type>,<reject_cause>
		// stat
		pszState = strstr(resBuffer, ",");
		if (pszState != NULL)
		{
...
int BG96::reportDevice()
{
	...
	memcpy(tempBuffer2+1, tempBuffer, 7);

	//Serving ECGI
	for(i=0;i<4;i++)
	{
		makeBuffer[10+i] = ((tempBuffer2[0+2*i]-'0') << 4) + (tempBuffer2[1+2*i]-'0');
	}

	//Get RSRP
	intValue = 0;

	if(getRSRP( &intValue ) == 0)
	{
		SWIR_TRACE(F("RSRP : %d\n"), intValue );
	}
	...
	SWIR_TRACE(F("SINR/10 : %d, SINR%%10 : %d\n"), intValue/10, intValue%10 );
	makeBuffer[17] = (( intValue / 10 ) << 4 ) + ( intValue % 10 );

	//Model Name Length
	makeBuffer[18] = 0x0F;	/* CodeZoo "M1-SMS-Test-Dev" */
	//Model Name TEST
	makeBuffer[19] = 'M';
	makeBuffer[20] = '1';
	makeBuffer[21] = '-';
	...
	SWIR_TRACE(F("TxPOWER/10 : %d, TxPOWER%%10 : %d\n"), intValue/10, intValue%10 );
	makeBuffer[40] = (( intValue / 10 ) << 4 ) + ( intValue % 10 );

	//Position coordinates
	for(i=0;i<11;i++)
		makeBuffer[41+i] = 0x00;

	//Reserve Maximum 20byte
	makeBuffer[52] = 0x00;

	if(socketCreate(0, _IP, _PORT) == 0) 
		SWIR_TRACE(F("Socket Create!!!\n"));

	/* Socket Send */
	if(socketSend(makeBuffer, sizeof(makeBuffer)) == 0){
...
int BG96::chkLWM2M()
{
	...
	sprintf(szCmd, "AT+QLWM2M=\"select\"");

	ret = sendATcmd(szCmd, resBuffer, sizeof(resBuffer), "\"select\",2", 10000);

	return ret;
}

int BG96::setLWM2M_Server()
{
	char    szCmd[40];
	char    resBuffer[16];
	int     ret;
	...
	sprintf(szCmd, "AT+QLWM2M=\"cdp\",\"106.103.233.155\",5783");

	ret = sendATcmd(szCmd, resBuffer, sizeof(resBuffer), "OK", 10000);

	return ret;
}

int BG96::selectLWM2M(void)
{
	char    szCmd[24];
	char    resBuffer[16];
	int     ret;
	...
	sprintf(szCmd, "AT+QLWM2M=\"select\",2");
...
int BG96::getUICCID(char* szUICCID, int nBufferSize)
{
	char    szCmd[16];
	char    resBuffer[24];
	int     ret;
	...
	memset(szUICCID, 0, nBufferSize);

	strcpy(szCmd, "AT+ICCID");
	ret = sendATcmd(szCmd, resBuffer, sizeof(resBuffer), "+ICCID: ");

	memcpy(szUICCID, resBuffer, strlen(resBuffer)-1);

	return ret;
}

int BG96::enableLWM2M()
{
	char    szCmd[24];
	char    resBuffer[16];
	int		ret;
	...
	sprintf(szCmd, "AT+QLWM2M=\"enable\",1");
...
int BG96::disableLWM2M()
{
	...
	sprintf(szCmd, "AT+QLWM2M=\"enable\",0");
...
int BG96::registerLWM2M()
{
	...
	sprintf(szCmd, "AT+QLWM2M=\"register\"");
...
int BG96::makeHashLWM2M(char *HashCode, int nBufferSize, char *ServiceCode)
{
	char    szCmd[40];
	char    resBuffer[48];
	int		ret;
	...
	sprintf(szCmd, "AT+QLWM2M=\"epns\",0,\"%s\"",ServiceCode);

	ret = sendATcmd(szCmd, resBuffer, sizeof(resBuffer), "OK", 5000);

	if(ret == 0){
		sprintf(szCmd, "AT+QLWM2M=\"epns\"");
		ret = sendATcmd(szCmd, resBuffer, sizeof(resBuffer), "epns\",\"", 5000);

		memcpy(HashCode, resBuffer, strlen(resBuffer)-2);
	}

	return ret;
}

int BG96::epnsEnable(char *HashCode)
{
	char    szCmd[48];
	char    resBuffer[16];
	int		ret;
	...
	sprintf(szCmd, "AT+QLWM2M=\"epns\",1,\"%s\"", HashCode);
...
int BG96::makeMBSPSEnable(char *ServiceCode, char *phoneNumber, 
		char *uiccid, char *deviceName)
{
	char	szCmd[100];
	char	resBuffer[16];
	int		ret;
	...
	uiccid_extrace = strlen(uiccid) - 6;

	sprintf(szCmd, "AT+QLWM2M=\"mbsps\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\"", 
			ServiceCode, phoneNumber, phoneNumber, uiccid + uiccid_extrace, deviceName );
...
int BG96::lwm2mInit()
{
	...
	_serial.setTimeout(61000);	/* MAX 1minute */

	do{
		ret = readATresponseLine(resBuffer, sizeof(resBuffer), 
				"\"STATE_READY\"", 2000);
