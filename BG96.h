...
class BG96
{
	public:
		...
		/*
		 * Must be performed once at the beginning of LGU+ Cellular connection
		 */
		...
		/*
		 * Check if the BG96 module is registered with LWM2M.
		 * If return 0, registered LWM2M
		 */
		int chkLWM2M(void);

		/*
		 * Set LGU+ LWM2M Server IP & Port (106.103.233.155:5783)
		 */
		int setLWM2M_Server(void);

		/*
		 * Select LWM2M
		 */
		int selectLWM2M(void);

		/*
		 * UICCID is read from the SIM card 
		 */
		int getUICCID(char* szUICCID, int nBufferSize);

		/*
		 * LWM2M Client Enable/Disable 
		 */
		int enableLWM2M(void);
		int disableLWM2M(void);

		/*
		 * LWM2M Client Register
		 */
		int registerLWM2M(void);

		/*
		 * Make LWM2M Client HashCode
		 * Input Service Code 4 characters
		 * !!!Warnning!!! 
		 * You need to execute enableLWM2M() and use it after rebooting.
		 */
		int makeHashLWM2M(char *HashCode, int nBufferSize, char *ServiceCode);

		/*
		 * epns Enable
		 */
		int epnsEnable(char *HashCode);

		/*
		 * make mbsps
		 */
		int makeMBSPSEnable(char *ServiceCode, char *phoneNumber, char *uiccid, char *deviceName);
